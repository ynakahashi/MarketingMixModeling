...
## データ生成
set.seed(123)
pars <- c(100, 5, 0.8, 0.2, 0.7, 0.7, 0.4, 0.4, 4, 1, 0.1)
dat <- simulate_y(pars)
```
...
うーん、`k`はともかく`s`が当たっていませんね。推定された値でのプロットを見てみましょう。
...
plot(dat$X_01, hill_transformation(dat$X_01, res_optim$par[1], res_optim$par[2]), 
     ...
     ylab = "")
legend("topleft", legend = c("k = 0.4, s = 4", "k = 0.58, s = 0"), 
...
ダメだーー
xを(0, 1)にスケールしても、X_01に限定してもダメだー
誤差分散を小さくしてもダメだー
filterを固定してもダメだー
むしろfilterしてなくてもダメだー
